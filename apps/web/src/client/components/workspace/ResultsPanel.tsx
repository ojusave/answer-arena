import { Badge, Button, Collapse, Group, Stack, Table, Tabs, Text, Tooltip } from "@mantine/core";
...
/** Collapsed advanced table/chart/CSV for intermediate users. */
export default function ResultsPanel({
  ...
  return (
    <section className="results-panel pg-arena-card">
      <button
        type="button"
      ...
      >
        <Text fw={650} size="sm">
          {COPY.app.advancedResults} {open ? "▾" : "▸"}
        </Text>
        <Text size="xs" c="dimmed">
          Score grid, cost chart, and CSV export
        </Text>
      </button>

      <Collapse in={open} keepMounted={false}>
        <Stack gap="md" mt="md">
                        ...
                        <Table.Tr key={combo.comboId}>
                          ...
                          <Table.Td>
                            <div
                              ...
                              aria-label={
                                score == null ? "Not scored" : `${score} out of 100`
                              }
                            >
                          ...
                          <Table.Td>
                            <ModelCell modelId={combo.embeddingModel} />
                          </Table.Td>
                          <Table.Td>
                            <ModelCell
                              modelId={combo.rerankModel}
                              muted={!combo.rerankModel}
                            />
                          </Table.Td>
                          <Table.Td>
                          ...
                          <Table.Td>
                            ${combo.avgCostPerQuestion?.toFixed(4) ?? "—"}
                          </Table.Td>
                          <Table.Td>
                            {combo.p50GenerationLatencyMs?.toFixed(0) ?? "—"} ms
                          </Table.Td>
                        </Table.Tr>
                      );
                    })}
